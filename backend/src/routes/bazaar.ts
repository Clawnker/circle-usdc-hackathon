 ...
 * List discoverable x402 services: internal specialists + verified external agents.
...
router.get('/discovery', async (req: Request, res: Response) => {
  try {
    ...
    const { services, externalTotal, verifiedCount } = await getAllDiscoverableServices({ limit, offset });
    res.json({
      services,
